...
export const INITIAL_DATABASE_SEED: MloHubDatabaseSchema = {
  version: 2,
  ...
  restaurants: RESTAURANTS.map((r): RestaurantEntity => ({
    ...r,
    ownerId: r.id === 'mama-amina-biryani' ? 'usr-chef-amina' : 'usr-admin',
    ownerName: r.id === 'mama-amina-biryani' ? 'Mama Amina Juma' : 'Manager',
    ...
    neighborhood: r.address.includes('Masaki')
      ? 'Masaki'
      : r.address.includes('Oysterbay')
      ? 'Oysterbay'
      : r.address.includes('Mikocheni')
      ? 'Mikocheni'
      : r.address.includes('Sinza')
      ? 'Sinza'
      : 'Dar es Salaam',
    isVerified: true,
    ...
    menu: r.menu.map((m) => ({
      ...m,
      restaurantId: r.id,
      priceNum: m.priceNum || (parseInt(m.price.replace(/[^0-9]/g, '')) || 15000),
