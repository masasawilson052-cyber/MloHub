...
CREATE POLICY "Admins have full access to payments"
  ...
  USING (
    EXISTS (
      ...
      WHERE p.id = auth.uid() AND (p.role = 'MLOHUB_ADMIN' OR 'SUPER_ADMIN' = ANY(p.roles))
    )
  );
...
CREATE POLICY "Restaurant staff can view payment events for their restaurant"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.payments p
      JOIN public.restaurant_memberships rm ON rm.restaurant_id = p.restaurant_id
      WHERE p.id = public.payment_events.payment_id
...
BEGIN
  ...
  IF p_collected_amount < v_payment.amount THEN
    -- Flag underpayment fraud attempt
    UPDATE public.payments
    SET status = 'FAILED',
        failed_at = clock_timestamp(),
        failure_reason = 'Amount mismatch: expected ' || v_payment.amount || ' TZS, received ' || p_collected_amount || ' TZS'
  ...
  INSERT INTO public.payment_events (
    ...
    v_payment.id, p_event_id, 'PAYMENT_CONFIRMED', p_provider, 'PAID', p_collected_amount,
    p_merchant_reference, p_gateway_reference, p_raw_payload, 'GATEWAY_WEBHOOK'
  );
  ...
  IF v_payment.reservation_id IS NOT NULL THEN
    ...
    SET status = 'confirmed',
        ...
        deposit_amount_tzs = v_payment.amount,
        updated_at = clock_timestamp()
