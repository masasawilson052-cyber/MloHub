...
CREATE TABLE IF NOT EXISTS discovery_funnel_events (
    ...
    session_id TEXT NOT NULL,
    user_id UUID REFERENCES profiles(id) ON DELETE SET NULL,
    dish_id UUID REFERENCES menu_items(id) ON DELETE SET NULL,
...
CREATE POLICY "Restaurant staff update own dish trust"
    ...
    USING (
        EXISTS (
            ...
            WHERE restaurant_memberships.restaurant_id = dish_trust_scores.restaurant_id
            AND restaurant_memberships.user_id = auth.uid()
            AND restaurant_memberships.status = 'ACTIVE'
        )
    );
...
CREATE POLICY "Restaurants read own discrepancy reports"
    ...
    USING (
        EXISTS (
            SELECT 1 FROM restaurant_memberships
            WHERE restaurant_memberships.restaurant_id = customer_discrepancy_reports.restaurant_id
            AND restaurant_memberships.user_id = auth.uid()
            AND restaurant_memberships.status = 'ACTIVE'
        )
    );
